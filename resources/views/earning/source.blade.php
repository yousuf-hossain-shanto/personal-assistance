...
@section('page_title', 'Earning Source')
                        ...
                        <table class="table table-stripped table-bordered table-hover">
                            ...
                            @foreach($sources as $source)
                                <tr>
                                    ...
                                    <td>{{ $source->title }}</td>
                                    <td>{{ $source->desc_excerpt }}</td>
                                    <td>
                                        <button class="btn btn-warning btn-edit" data-id="{{ $source->id }}" data-title="{{ $source->title }}" data-description="{{ $source->description }}">
                                            Edit
                                        </button>
                                        <button class="btn btn-danger btn-delete" data-id="{{ $source->id }}">Delete
                                        </button>
                                    </td>
                                </tr>
                            @endforeach
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">New Earning Source</h4>
                    <button type="button" class="float-right close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span></button>
                </div>
                <form action="{{ route('earning-source.store') }}" method="post">
                    @csrf
            ...
            $(document).ready(function () {
                $(document).on('click', '.btn-edit', function (e) {
                    ...
                    datas.forEach(function (data) {
                        if (data.name == 'id') {
                            var url = '{{ url('earning-source') }}/' + data.value;
